...
def infer(model, path, detections_file, resize, max_size, batch_size, shuffle=False, mixed_precision=True, is_master=True, world=0,
          annotations=None, is_validation=False, verbose=True, rotated_bbox=False, score_threshold=0.3):
    ...
    if rotated_bbox:
        data_iterator = RotatedDataIterator(path, resize, max_size, batch_size, shuffle, stride,
                                            world, annotations, training=False)
    else:
        data_iterator = DataIterator(
            path, resize, max_size, batch_size, shuffle, stride,
            world, annotations, training=False)
