...
from utils_box import iou, iou_faster, load_cpp_extensions
...
def snap_to_anchors_rotated(boxes, size, stride, anchors, num_classes, device, anchor_ious):
    ...
    if torch.cuda.is_available():
        cpp_module = load_cpp_extensions()
        iou = cpp_module.iou
    #
    overlap = iou(boxes_rotated.contiguous().view(-1), anchors_rotated.contiguous().view(-1))[0]
    # FILL THE NMS, NMS_ROTATED, IOU, IOU_ROTATED FUNCTION
    # cpu<->gpu conversion may be required
    # overlap = iou_faster(boxes_rotated.contiguous(), anchors_rotated.contiguous())
    # Keep best box per anchor
    overlap, indices = overlap.max(1)
...
def decode(all_cls_head, all_box_head, stride=1, threshold=0.05, top_n=1000, anchors=None, rotated=False):
    ...
    if torch.cuda.is_available():
        cpp_module = load_cpp_extensions()
        return cpp_module.decode(all_cls_head.float(), all_box_head.float(),
...
def nms(all_scores, all_boxes, all_classes, nms=0.5, ndetections=100):
    ...
    # nms_module = load(name='nms_module', sources=['csrc/extensions.cpp', 'csrc/cuda/nms.h', 'csrc/cuda/nms.cu'], verbose=True)
    if torch.cuda.is_available():
        cpp_module = load_cpp_extensions()
        return cpp_module.nms(all_scores.float(), all_boxes.float(), all_classes.float(),
...
def nms_rotated(all_scores, all_boxes, all_classes, nms=0.5, ndetections=100):
    'Non Maximum Suppression'
    # nms_rotated_module = load(name='nms_rotated_module', sources=['csrc/extensions.cpp', 'csrc/cuda/nms_iou.h', 'csrc/cuda/nms_iou.cu'], verbose=True)
    if torch.cuda.is_available():
        cpp_module = load_cpp_extensions()
        return cpp_module.nms(all_scores.float(), all_boxes.float(), all_classes.float(),
    ...
    for batch in range(batch_size):
        ...
        for i in range(ndetections):
            ...
            #inter = iou_faster(boxes_rotated.contiguous(), boxes_rotated[i, :].contiguous(), inter_only=True)
            inter = inter.squeeze()
            criterion = ((scores > scores[i]) |
                         (inter / (areas + areas[i] - inter) <= nms) |
                         (classes != classes[i]))
...
# def init_cpp_modules():
#     return load(name='csrc_module',
#                 sources=['csrc/extensions.cpp', 'csrc/cuda/decode_rotate.h', 'csrc/cuda/decode_rotate.cu', 'csrc/cuda/decode.cu', 'csrc/cuda/decode.h','csrc/cuda/nms_iou.cu','csrc/cuda/nms_iou.h','csrc/cuda/nms.cu','csrc/cuda/nms.h'],
#                 verbose=True)
